...
- [ ] **Paper the POLEKA inclusion, and the Alex answer with it.** Both go in the same one-line message: POLEKA is in the Foundation at four brands and no change to fee, and Alex is the parent endorsing selected lines. Clause 4's rejection rule leans on the approved guide matching what the client actually told us, so a verbal answer that shapes four deliverables needs to exist in writing. (tsa)
- [ ] **Umbrella or endorsement? The biggest open question on the job.** The packs say Alex endorses selected lines; **the client's own marketing runs Alex Foods as a consumer master brand** carrying all four. Those give opposite instructions and the answer decides whether TSA builds **four brand pages or one Alex Foods presence** — which reshapes deliverable 3, the content calendar and the whole of Package A. Not a detail, and it should be settled before any page gets created. (tsa)
...
- [ ] **One client answer left, down from four.** Whether they hold rights to the "Cola Cola" artwork TSA would be pushing in paid campaigns. **Three are closed:** both 2MAN designs are live (a sub-brand rule, not a question), there are no colour codes or font names to collect because setting them is the deliverable, and **"what is Alex" answered 18 Sep — parent company endorsing selected lines**, so no house voice and no house palette. Full detail in [[Alex Foods Brands]]. (tsa)
- [ ] **Ask for the file by name: `ashkal mucup`.** Taking the Canva mockup apart on 18 Sep exposed that its 2MAN logo is **a screenshot of a PDF viewer with the filename still visible**, and its AlRawy logo is a screenshot with the iOS status bar still in it. **Every route the artwork has taken to TSA has been a photograph of a file somebody had open.** Stop asking for "vector files" in the abstract and ask for that document by name, plus the equivalent for the other three. (tsa)
- [ ] **Send the actual logo FILES, not screenshots.** The AlRawy and 2MAN marks arrived 18 Sep as iPad screen captures of files open in the Files app. Clean and isolated, far better than reading a mark off a pack, **but still raster** — they do not close material #2 and cannot be used for print or at large scale. **The screenshots prove the source files exist on the client's device**, so the ask is now precise: send the `.ai`, `.eps`, `.svg` or vector `.pdf` itself, for all four brands. (tsa)
...
- [ ] **Chase the 7 client materials — the only gate still actually holding.** **Packaging artwork and SKU names are in** as of 18 Sep. Still outstanding: **vector logo files** (mockup renders are not vector), **product photos or samples** (packaging renders are not product photography, and every visual deliverable depends on this one), Meta admin access, Google account, GBP details. (tsa)
...
### Completed Tasks
- [x] **Design System v1.0 delivered as a PDF** — `Alex Foods - Design System v1.0.pdf`, 19 pages, A4, generated from `design-system/` and carrying its own sign-off page. Closes Foundation deliverable 2 and **closes Phase 1 entirely**. Pending the client's written approval, not pending further work. (18 Sep 2026)
