...
- `packshots/` — **the client's own product mockups for all four ranges**, received 19 Sep, cropped and keyed to transparency, with individual packs sliced out in `packshots/single/`. See `packshots/packshots.md` — **it carries the resolution warning.**
...
- `design-system/` — the build for **both** PDFs: the colour maths, the two generators (`gen.py` for the Design System, `kit.py` for the kit), the fonts, and the traps worth not rediscovering. See `design-system/design-system.md`.
...
- [[Content Plan]] — **Foundation deliverable 5 and the strategy behind the whole term.** How the marketing playbook translates to a product sold in shops, the funnel Alex Foods actually has, the supply gap between the kit's posting rhythm and the contract's 20 assets, the three months, month 1 specified asset by asset, and the month-3 engagement goal with its verdict thresholds.
...
>
> **And this version carries no Egyptian flag ribbon.** `logo-alex-seal.jpg` has one. **Two versions of the master mark now exist, and the Foundation has to say which one governs.** See `logo-vector/logo-vector.md`.
