...
RANGES = [
 ('BeBo','بيبو','Powdered drink sachets','مساحيق المشروبات','#1BA34C','logo-bebo-transparent.png',
  'Talks to the mother. The characters already own the child.',
  [('Peach','#E4762A'),('Mango','#F2A00C'),('Apple','#2C8C3B'),('Pineapple','#C6D42E')]),
 ('AlRawy','الراوي','Juice and nectar pouches','عصائر ونكتار','#1B4F9C','logo-alrawy-transparent.png',
  ...
  [('Cocktail','#D6006E'),('Apple','#00A3E0'),('Guava','#1BA34C'),('Mango','#F07F13')]),
 ('2MAN','تومان','Ice pops, two live lines','مثلجات','#29ABE2','logo-2man-transparent.png',
  'Talks to the kid directly. They spend their own pocket money.',
  [('Blue','#29ABE2'),('Red','#E1251D'),('Green','#8DC63F'),('Orange','#F07F13')]),
 ('POLEKA','بوليكا','Jelly candy','حلوى جيلي','#EC008C','logo-poleka-transparent.png',
  'Built to be asked for by name, out loud, in a shop.',
  [('Apple','#1BA34C'),('Mango','#FFF200'),('Strawberry','#EC008C')]),
]
...
def page(body, label=''):
    ...
    p(f'<section class="page">{body}'
      ...
      f'<span class="fr">{label} — PAGE {PAGES[0]} / 6</span></footer></section>')
...
<div class="lk" style="background:{RED};color:{PAPER}"><img src="../logos-transparent/logo-alex-lockup-transparent.png"><span>ON RED — CAMPAIGNS</span></div>
<div class="lk" style="background:{MIST};color:{INK}">
  <div class="minis">{''.join(f'<img src="../logos-transparent/{f}">' for n,a,d,da,c,f,w,fl in RANGES)}</div>
...
rngc=''.join(f'''<div class="rsw"><i style="background:{c}"></i><b>{n}</b>
 <span class="ar">{a}</span><span class="mono">{c}</span></div>'''
 for n,a,d,da,c,f,w,fl in RANGES)
...
<div class="promise">{corners()}<div class="grid"></div>
 ...
 <div class="pbadge">ALEX<br>FOODS<br><em>—</em><br>ALEX</div></div>
<p class="fn"><b>This is the client's own line</b>, already running across their market creative. It is recorded here so nobody invents a second one by accident. <b>A real brand line is Foundation deliverable 4</b> and this is not it.</p>''','BRAND IDENTITY')
...
for bg,cap_ar,kind,note,fg in [
 ...
 ('#1BA34C','قوليلهم آه.','FIELD — A PRODUCT POST','Flavour colour, character leads.',None)]:
    kl=False
...
<div class="two">
 ...
 <span class="cap">PRIMARY — ALEX NAVY</span><p>Use this everywhere by default. Holds on a light feed and a dark one.</p></div>
 <div class="card ctr"><div class="pfp" style="background:{RED}"><img src="../logos-transparent/logo-alex-lockup-transparent.png"></div>
 <span class="cap">ALT — SYSTEM RED</span><p>Campaigns and launches, or when the account needs to break a grid.</p></div>
</div>
...
for rn,ar,d,dar,c,f,who,fl in RANGES[:2]:
    ...
    <span class="tpk">{rn} · {lab} · <span class="mono">{fld}</span></span>
...
st=''
for rn,capar,fld,logo in [('2MAN','برّه أحلى.','#29ABE2','logo-2man-transparent.png'),
                          ('POLEKA','مين معاك النهاردة؟','#EC008C','logo-poleka-transparent.png'),
                          ('AlRawy','كل يوم.','#00A3E0','logo-alrawy-transparent.png')]:
    fg,cr,kl = best(fld)
    ks = f'text-shadow:0 0 1mm {INK};' if kl else ''
...
<p>In Canva or Figma: flavour field as the background layer, the pack cut-out from <span class="mono">logos-transparent/</span> on top, red bar and ALEX FOODS at the corner, Arabic at the bottom in Plex Sans Arabic 700. <b>Write the Arabic first, then fit the English.</b> That is the whole formula.</p></div>
...
<div class="sign">{corners()}<div class="grid"></div>
 <p class="ar">أليكس فودز</p><p class="mono">BRAND &amp; SOCIAL KIT · v1.1 · 18.09.2026</p>
...
/* lockups */
.locks{{display:grid;grid-template-columns:repeat(4,1fr);gap:3mm;margin-top:3mm}}
.lk{{height:22mm;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2mm;padding:3mm}}
...
.csw .chip{{height:11mm;width:100%;display:block;margin-bottom:2.5mm}}
...
.tyl p.d1{{font-size:21pt;font-weight:700;margin:0;line-height:1.32;color:{INK}}}
...
.tf{{border:0.25mm solid {SILVER};padding:2.2mm 3.5mm}}
...
/* the promise */
.promise{{position:relative;background:{RED};color:{PAPER};padding:4.5mm 7mm;overflow:hidden;
 display:flex;align-items:center;justify-content:space-between;gap:6mm}}
.promise .px{{position:relative;z-index:2}}
.promise .ar{{font-size:15pt;font-weight:700;margin:0}}
.promise .en{{font-size:7.5pt;font-weight:700;letter-spacing:.2em;margin:1.5mm 0 0}}
.pbadge{{position:relative;z-index:2;width:19mm;height:19mm;border-radius:50%;border:0.3mm solid {PAPER};
 display:flex;flex-direction:column;align-items:center;justify-content:center;flex:none;
 font-size:5.2pt;font-weight:700;letter-spacing:.12em;line-height:1.5;text-align:center}}
.pbadge em{{font-style:normal;opacity:.6}}
/* positioning */
...
.pfp{{width:23mm;height:23mm;border-radius:50%;margin:1mm auto 3mm;display:flex;align-items:center;justify-content:center;overflow:hidden}}
...
.tp{{position:relative;height:50mm;padding:4mm;display:flex;flex-direction:column;overflow:hidden}}
.tph{{position:relative;z-index:2;display:flex;align-items:center;gap:1.5mm}}
.tph i{{width:0.9mm;height:3.2mm;display:block}}
.tph span{{font-size:5.6pt;font-weight:700;letter-spacing:.14em}}
...
<title>Alex Foods — Brand &amp; Social Kit v1.1</title>
