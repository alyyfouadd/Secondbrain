...
type: reference
---
# logos-vector — the real Alex Foods master seal
...
`alex-seal-master.pdf` — the client's own file, unmodified.
...
| File | What it is |
|---|---|
| `alex-seal-master.pdf` | **The master.** The client's original. Never edit it, never re-save over it |
| `alex-seal-flat.svg` | Converted to SVG for the HTML → PDF build in `design-system/`. 116 paths, 1:1 with the source |
...
## Folder consolidated, 20 September 2026

Two sessions received the same vector seal on 19 September and filed it in two folders — `logos-vector/` and `logos-vector/`. **The PDFs were byte-identical**, so one copy was kept and everything else merged here, under the plural name that matches its sibling `logos-transparent/`.

**Two SVG conversions survive, deliberately, because they were produced by different converters:** `alex-seal-master.svg` and `alex-seal-flat.svg`. **Check whichever one you use before shipping it** — one known defect is recorded above, the x of "Alex" not surviving conversion. **The PDF is the master; every SVG is a working copy.**
