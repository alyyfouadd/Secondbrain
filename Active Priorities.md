...
- [ ] **Decide the month-1 start condition before it goes in the delivery message.** It currently reads *"the later of written Foundation approval or the arrival of usable product photography **and vector logo files**."* **As written it can never fire** — and photography is no longer a gate either, since TSA produces it ([[Decisions]] #13). **It should almost certainly name written approval alone now.** The sentence also sits in [[Alex Foods]] and [[Foundation Roadmap]]; all three change together. (tsa)
- [ ] **Correct the master colour in all three generators.** **The eight design fixes and the seal repointing are DONE** — you approved them 20 September and both documents re-rendered ([[Brand Book Spec]] §6b). **The colour correction was not in that pass and is still outstanding:** `book.py`, `kit.py` and `gen.py` all still carry `#0A0378`, which [[Colour System]] §11 puts **ΔE2000 9.80 off** the authored CMYK, and `book.py` and `kit.py` still use `#05004B` on hero panels and the sign-off block — **a value that was never a brand colour, only a gradient stop of a render.** Source-code change; needs your go-ahead. **Fold it into the same render as the POLEKA line below, so the book is built once.** (tsa)
...
- [ ] **DO NOT delete the branches yet. Two of them carry work that is not on `main`, and the line that used to sit here said the opposite.** Re-verified 21 September against the live remote: there are **thirteen** session branches, not nine, and **eleven are contained in `main`** — but **`claude/alexfoods-color-kit-guidelines-txx208` (3 commits) and `claude/foundation-requirements-oyjwf3` (10 commits, ~1,070 lines) are not.** Between them they hold **Mahmoud's agreed rates — the single biggest blank in [[TSA Money]] — the `Month 1 Asset Brief`, the Arabic company brief, the voice cast, the school-campaign plan, the BeBo back-of-pack shot, and rows in [[Decisions]], [[Delivery Register]] and `retired.tsv`.** **Deleting on the old instruction would have destroyed all of it.** **Merge both into `main` first, then delete.** (meta)
- [ ] **The remote cannot delete a branch from this session — GitHub returns 403 to a ref deletion, three attempts, and it is a token-scope limit rather than a network fault.** So *"deleted on both ends"* in the 20 and 21 September logs is **not what happened** — `claude/alexfoods-marketing-plan-4wt8h0` is still sitting on the remote right now. **Branch deletion is an Aly action in Working Copy or Safari, always**, and no session should record one as done. (meta)
