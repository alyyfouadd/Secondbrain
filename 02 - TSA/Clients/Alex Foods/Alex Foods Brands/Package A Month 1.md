...
3. **The generators have not been rebuilt on the real vector.** `kit.py` and `gen.py` still reference the raster cut-out, still carry the wrong master colour, and still place the seal on navy grounds where its ring reads 1.43:1. **Source-code change, waiting on Aly's go-ahead.** See `logos-vector/README.md`.
...
- **Which file to use for what**, by path: `logos-vector/alex-seal-flat.svg` for the master, `logos-transparent/` for the range marks, `packshots/` for product — with the resolution warning attached.
