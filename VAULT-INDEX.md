...
> **A real example of why this rule exists, 19 September 2026 — and it is history, not current state.** Four asset folders each carried an index called `README.md`, and the root vault-setup doc is also `README.md`. **Three `[[README]]` wikilinks all meant the root one, and Obsidian had five files to choose from.** The link resolved to whichever it felt like, so the four asset indexes were renamed to match their folders. That was safe only because nothing wikilinked them.
>
> **The rename did not survive.** The 20 September move of Alex Foods into `Clients/` rebuilt those folders with `README.md` files, silently reverting it, and this paragraph went on describing the renamed state for four days. **All four are `README.md` on disk today, which the asset-folder carve-out above now says is deliberate and correct** — the collision is handled by the carve-out's rule that an asset README is referenced by path and never wikilinked, not by giving it a unique name. **`README.md` at the root keeps its name, and `[[README]]` resolves to exactly one file because it is the only one anything links to.** *(Caught 24 September by checking the disk against this paragraph.)*
...
`05 - Resources/vault-check/check.py` runs two families of check over the whole vault. **Structural**, from `structure.py`: PDF references, frontmatter, wikilink resolution, the no-wikilinks-in-a-log rule, folder-index coverage, and orphans. **And the retired-claim check**, which walks every `.md` and `.py` against `retired.tsv` and fails on any live statement of something a decision has retired — the one that catches a correction the generators still hold their own copy of. `python3 "05 - Resources/vault-check/check.py"` from the repo root; exit 0 clean, 1 with findings. Full documentation sits beside it in that folder's `README.md`. **Run it before any checkpoint that touched more than one note, and never render or commit on a failing check.** *(It used to live at `.claude/tools/vault-check.py`; folded in as `structure.py` on 20 September, because two checkers with the same name in two places is the duplication the tool exists to catch.)*
