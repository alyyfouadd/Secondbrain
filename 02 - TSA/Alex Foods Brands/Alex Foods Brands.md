...
- **`Alex Foods - Brand Foundation v1.0.pdf` — THE BOOK. 34 pages, 25 sections, bilingual, 8.0 MB.** Built 19 September from `design-system/book.py`, which is the single source for every section's content. **17 sections DEFINED and complete; 5 carry an honest waiting chip naming what they wait on and who from.** This is the only document the client receives. See [[Brand Book Spec]] §5c.
...
- `Alex Foods - Slogans v1.0.pdf` — **the ten lines on their own**, 2 pages, 0.32 MB. Rendered from the book with `render(['10'])`, **not a second generator**, so the text exists in one place only. For reading out loud and for putting in front of the client.
