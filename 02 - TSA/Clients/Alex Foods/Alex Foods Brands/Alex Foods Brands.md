...
- [[Mahmoud Engagement]] — **the commercial conversation around the brief:** the seven things TSA needs back before he starts, the walk-away reasoning, and the two messages that get them. **The kit and the graphics are quoted separately on purpose** — merged, neither number can ever be checked again.
- [[Mahmoud Kit Brief]] — **the brief for the designer laying out the client-facing identity document.** Scope, the ten unbreakable rules, the eight cheap-tells as a reject list, every colour and type value he needs, what TSA supplies, the three decisions Aly still owes, and the terms. **A derived export that owns nothing** — re-derive it when a value changes upstream rather than patching it. [[Decisions]] #36 and #37.
- `Alex Foods - Brand Guidelines and Colour Kit v1.0.pdf` — **the guidelines and colour kit as a standalone, 10 pages, 6.4 MB.** Sections 03 to 07 only: brand architecture, colour system, type system, logo and seal use, and the four ranges. Rendered 20 Sep from `design-system/book.py` with `render(['03','04','05','06','07'])`.
  **Cover, contents and sign-off are deliberately excluded.** The book's cover reads *BRAND FOUNDATION* dated 09.10.2026, the contents page lists all 22 sections, and the sign-off approves the whole stage — **all three would claim this extract is the complete 20,000 EGP deliverable.**
  **One known defect: Alex Navy (`#0A0378`, governed in [[Colour System]]) in §04 is wrong by ΔE2000 9.80** against the client's real vector — see [[Decisions]] #14. **Fine for a designer starting work; not fine in front of the client until the master colour is re-cut.**
...
- [[Local SEO — Alexandria]] — **Foundation deliverable 8, book §20.** Who actually searches for a factory: traders first, then brand names, then directions. What is already online and where it disagrees with itself (a lapsed website, a second Facebook address, two legal names, a one-digit WhatsApp mismatch). The single set of business data every surface copies. The keyword map, with the fresh-juice-shop trap excluded on purpose. The digital-shelf competitor read, and the monthly Google profile routine Package A pays for.
