...
**The probe's own exclusion list is a trap, and a broken probe reads as a catastrophe rather than a bug.** The overflow probe skipped the footer by testing `className`, but the footer is a `<footer>` **tag** with no class, so it was measured on every page: the probe reported all 39 pages overflowing at a suspiciously uniform 1089px. **A real 1115px overflow was sitting inside that noise and would have shipped.** Skip by `e.tagName==='FOOTER' || e.closest('footer')`, and treat *every page fails by the same number* as a defect in the probe, never in the document. **Prove the probe works by running it against a render you already know is clean.**

**The rendered page order is not the order sections were written in.** `render()` sorts by section id, so walking `SEC` in insertion order to map "page 28" to a section gives a confident, wrong answer — it pointed at section 12 and sent the fix to a section that had nothing wrong with it. **Read the map out of the generated HTML** (split on `<section class="page"`, pull each page's `.eb > span` label). The pages are `<section>` elements, not `<div>`, which is its own quiet way to make a regex return zero matches.

**Trap 5 bit again, in section 17.** A table cell written as `<td class="num">` matched `.num > b` from the stat-card grid and rendered its digits at 20pt. Same shape as the `.tl` collision: **a short, generic class name used for two unrelated things.** And reusing `.cap` for a new table silently applied `td:nth-child(3){width:26mm}`, crushing a content column into a ribbon. **Never borrow a modifier class for a new table because it looks close enough — the widths are part of it.**
